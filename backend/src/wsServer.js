...
// dialogueManager imports emitToClient from wsServer,
// so we defer the dialogueManager import to first use.
...
// ---------------------------------------------------------------------------
// Client registry — module-level so emitToClient is usable before initWsServer
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Initialiser — called from index.js with the shared http.Server
// ---------------------------------------------------------------------------

/**
 * Attach the WebSocket server to an existing http.Server instance.
 * Both REST and WebSocket share a single port — required for Railway.
 *
...
export function initWsServer(httpServer) {
  ...
  wss.on('connection', (ws) => {
    ...
    // Send the client its own ID immediately on connect.
